# Ukázková jednotka - STUDIO jednotka

> Provoz interní znalostní báze - jednotka drží obsah, který čte celý tým, a hlídá, aby v něm nezůstávalo zastaralé.

## Zařazení

- **Tenant:** NorthStar Lab (vlastník harnesse a portfolia STUDIO jednotek, do kterého tenhle projekt patří).
- **Klasifikace:** Internal (META | Internal | Client | Personal). Autoritativní zdroj typu projektu - řídí management style (compliance, NDA, cadence, secrets discipline, komunikační tón). Lokace na disku je organizační konvence, ne klasifikace.
- **Typ:** Projekt (Průvodce | Asistent | Projekt | Mini-produkt | Automat, per OR-03 v6). Jaký tvar práce STUDIO jednotka má - vícekrokový workflow / doménový společník s human-in-the-loop / ohraničený projekt s plánem a koncem / featura-nástroj-artefakt / autonomní běh bez člověka po ruce. Čistý label, NEřídí chování (validátor je type-blind); klasifikaci nezaměňovat s typem.
...
Jednotka existuje jako ukázka. Je to výsledek zkopírování `scaffold/studio-template/`
a vyplnění hlavičky - přesně to, co při zakládání nové jednotky dělá orchestrátor.

- **Cílový výsledek:** znalostní báze, ve které se dá spolehnout na to, že platí, co je v ní napsané.
- **Zdroje pravdy:** `operations/status.md` (stav), `operations/platform-baseline.md` (co jednotka převzala z platformy).
- **Scope boundary:** provoz obsahu ano, nástroje pro jeho ukládání ne.
- **Citlivá data:** žádná.

Reálná jednotka má v tomhle místě výstup Discovery Q&A a je to zdroj pravdy pro každý
brief subagentovi (OR-01). Když je prázdný, jednotka není ready k exekuci.
...
**Co do jednotky patří:** revize živého obsahu, vyřazení zastaralého, zápis nových poznatků, kontrola odkazů.

**Co se dá zaměnit, ale patří jinam:**
- Výběr nástroje pro ukládání znalostí → jednotka, která vlastní systémovou architekturu, protože je to rozhodnutí o integraci, ne o obsahu.
- Text, který uvidí zákazník → jednotka, která vlastní produktovou dokumentaci, protože publikum je jiné a žánr taky.
